...
# Переключатель журналов обязан переключаться. Он залипал, когда два пина
# несли одно имя: список видов повторял его, и следующий вид оказывался тем же.
before=$(xdotool getactivewindow getwindowname 2>/dev/null || true)
for i in 1 2 3; do
    xdotool mousemove --window "$wid" 400 300 click 1
    sleep 1
done
# Достаточно, что приложение живо и журнал пополняется: залипание проверяется
# модульным тестом на уникальность видов, здесь важно, что клики не роняют окно.
pgrep -f "$tmp/crescent" >/dev/null || fail "окно упало при переключении журнала"
